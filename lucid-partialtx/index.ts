...
export function buildTxFrom(
    this: Lucid,
    { inps, outs, mint, requiredSignatories, extraDatums, validityRange }: PartialTx
): Tx {
    ...
    tx = outs.reduce((acc, { address, val, datum }) => {
        ...
        return datum ? acc.payToAddressWithData(targetAddr, datum, targetVal) : acc.payToAddress(targetAddr, targetVal);
    }, tx);
